...
EMSCRIPTEN_BINDINGS(libLCE) {
    ...
    .function("getOrCreateDirByPath", &lce::fs::Filesystem::getOrCreateDirByPath, emscripten::allow_raw_pointers())
     // .class_function("windowsToUnixDelimiter", &lce::fs::Filesystem::windowsToUnixDelimiter) emscripten doesn't like these... nice
     // .class_function("unixToWindowsDelimiter", &lce::fs::Filesystem::unixToWindowsDelimiter)
    ...
    // .constructor<std::wstring, std::vector<uint8_t>, uint32_t, lce::fs::Directory *>(emscripten::allow_raw_pointers()) // error
    ...
    .constructor<uint32_t, uint32_t,
        std::unordered_map<uint32_t, std::string>, std::vector<lce::loc::Language>>()
