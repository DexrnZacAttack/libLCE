...
mod tests {
    ...
    fn it_works() {
        ...
        for entry in WalkDir::new("testing/examples").into_iter().filter_map(Result::ok) {
            let path = entry.path();
            if path.is_file() {
                if let Ok(rel_path) = path.strip_prefix("testing/examples") {
        ...
        let write_save = saves::compress_save::<BigEndian>(saves::write_save::<BigEndian>(test, 11, 11), SaveCompression::ZLIB);
