...
namespace lce::fs {
    File* Directory::createFile(const std::wstring& name, std::vector<uint8_t> data) {
        if (children.count(name))
            return nullptr; // file exists already... can't have 2 of them (although we are NOT case-insensitive)
    ...
    /// Allows us to add any type of object that extends FSObject by relying on us to make the object instead of the
    /// Directory class Useful, although it does break the "no orphaned file should exist" guideline that I wanted to
    /// follow... However, I think things such as takeChild also break that, as technically you are the sole owner of
    /// the child that you took.
    ...
    bool Directory::removeChild(const std::wstring& name) { return children.erase(name); }
    ...
    bool Directory::moveChild(const std::wstring& name, Directory* to) {
        if (!to || to == this || to->children.count(name))
            return false; // if to is nullptr, or to is literally this, or to contains an object with the same name
                          // already
    ...
    size_t Directory::getSize() const {
        ...
        for (const auto& [name, child] : children) {
            if (!child->isFile())
                if (const Directory* dir = dynamic_cast<const Directory*>(child.get()))
                    s += dir->getSize();
